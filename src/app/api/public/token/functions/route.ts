...
import { contractsInterfaces, contractsNames } from "@/contracts/interfaces";
import { NextResponse } from "next/server";
import { StatusCodes } from "http-status-codes";
import { cairoInputsFormat } from "@/utils/cairoInputsFormat";

async function handler(req: NextRequestWithAuth) {
  const erc20AllowedFunctions = contractsInterfaces[contractsNames().ERC20EventCurrency]?.abi?.filter(i => i.type === "interface").flatMap(i => i.items);
