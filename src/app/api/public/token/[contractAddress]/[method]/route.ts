...
import { contractsInterfaces, contractsNames, getGaslessTransactionCallData } from "@/contracts/interfaces";
...
const erc20Abi = contractsInterfaces[contractsNames().ERC20EventCurrency]?.abi || [];
const erc20AllowedFunctions = erc20Abi.filter(i => i.type === "interface").flatMap(i => i.items);

async function handler(req: NextRequestWithAuth, { params: { contractAddress, method } }) {
  const body = await req.json();
  const { userId, developerId } = req;

  const functionObject = erc20AllowedFunctions.find((m) => m.name === method);
  const inputsExists = functionObject.inputs.every((input) => body[input.name] !== undefined);

  if (!inputsExists) {
    return NextResponse.json(
      {
        error: "Missing parameters",
        requiredParameters: cairoInputsFormat(erc20AllowedFunctions.find((m) => m.name === method).inputs)
      },
  ...
  const contract = connectToContract({ address: contractAddress, name: "ERC20EventCurrency" });
